...
void slee_P() {                                                         // ЦИКЛ ДЛЯ СНА
  ...
  } else if (v <= 4050 && v > 3850) {
    n = 552000;  //10 minutes
  
  } else if (v <= 3850 && v > 3750) {
    n = 859500;  //15 minutes

  } else if (v <= 3750 && v > 3650) {
...
long readVcc() {                                                       //функция чтения внутреннего опорного напряжения, универсальная (для всех ардуин) vbyen
  float my_vcc_const = 1.100;   // константа вольтметра
...
void null_eeprom() {
  ...
  eeprom_null.unix_time = 1746738001;
...
void set_data_time() { // установка времени
  ...
  // Парсинг и проверка года (формат YY, диапазон 2004-2038 или другой допустимый)
  year = (_data_time.substring(0, 2)).toInt();
  year += 2000;
  if (year < 2004 || year > 2038) { // Проверка на допустимый диапазон
